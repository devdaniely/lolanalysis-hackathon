...
def get_games_tournament_stage(tournament_id, stage):
  ...
  ranked_dict = replace_team_id_with_team_name(sorted_dict)
  print(ranked_dict)




def replace_team_id_with_team_name(input_dict):
  new_dict = {}
  for key, value in input_dict.items():
    if key in team_mappings:
      new_key = team_mappings[key]["name"]
...
'''
def get_ratings(initial_team_ids, tournament_game_ids):
  ...
  # init team dict
  team_dict = dict()
  for team_id in initial_team_ids:
    team_dict[team_id] = {
      "elo": 1200,
      "prox_score": 0,
      "prox_sum": 0,
  ...
  for tournament_game_id in tournament_game_ids:

    # skip if game missing
    if tournament_game_id not in esportsGameIdMappings.keys():
      continue

    game_map = esportsGameIdMappings[tournament_game_id]
